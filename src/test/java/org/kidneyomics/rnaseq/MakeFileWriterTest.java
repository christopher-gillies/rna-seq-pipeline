...
public class MakeFileWriterTest {
	...
	@Test
	public void testKallisto() throws Exception {
		...
		File reference = new File("/tmp/transcriptome.fa");
		File kallisto = new File("/tmp/kallisto");
		...
		when(applicationOptionsMock.getReferenceTranscriptome()).thenReturn("/tmp/transcriptome.fa");
		...
		makeFileWriter.writeMakeFile(Mode.KALLISTO);
		...
		Resource r = new ClassPathResource("MakefileKallisto");
